...
from core.datasets.safety.orbench import OrBench
from random import choices
from core.attacks.helpers.attackers.strategy_attacker import Strategy
from pathlib import Path

safety_df = OrBench().sample_randomly(n=200, random_state=42)
...
def get_base_folder(index):
    if index == 0:
        return (
            "datasets/crescendo_orbench_qwen3n_vs_circuitbreaker__async_batch_bagel_eval_3repl_3x8x5"
        )
    elif index == 1:
        return "datasets/crescendo_orbench_qwen3n_vs_dertains__async_batch_bagel_eval_3repl_3x8x5"
    elif index == 2:
        return "datasets/crescendo_orbench_qwen3n_vs_door__async_batch_bagel_eval_3repl_3x8x5"
    elif index == 3:
        return "datasets/crescendo_orbench_qwen3n_vs_guardrnoh__async_batch_bagel_eval_3repl_3x8x5"
    elif index == 4:
        return (
            "datasets/crescendo_orbench_qwen3n_vs_llamainstruct__async_batch_bagel_eval_3repl_3x8x5"
        )
    elif index == 5:
        return "datasets/crescendo_orbench_qwen3n_vs_cbllama__async_batch_bagel_eval_3repl_3x8x5"
    elif index == 6:
        return "datasets/crescendo_orbench_qwen3n_vs_guardqnoh__async_batch_bagel_eval_3repl_3x8x5"
    elif index == 7:
        return "datasets/crescendo_orbench_qwen3n_vs_nemor__async_batch_bagel_eval_3repl_3x8x5"
    elif index == 8:
        return "datasets/crescendo_orbench_qwen3n_vs_nemoq__async_batch_bagel_eval_3repl_3x8x5"
    elif index == 9:
        return (
            "datasets/crescendo_orbench_qwen3n_vs_cbadimixed2000__async_batch_bagel_eval_3repl_3x8x5"
        )
    elif index == 10:
        return (
            "datasets/crescendo_orbench_qwen3n_vs_cbadicbmixed2t001__async_batch_bagel_eval_3repl_3x8x5"
        )
    else:
...
async def run_one(row, agent_index=0, shared_session=None, i=0):
    ...
    with MessageFileLogger(
        output_folder_name=f"{base_folder}/crescendo_orbench_{row['id']}{suff}", buffer_max_size=20
    ) as logger:
        ...
        with open(f"{base_folder}/crescendo_orbench_{row['id']}{suff}/info.txt", "w+") as f:
...
async def run_all(safety_df, max_concurrent: int = 5, _agent_index=0, indices=None):
    ...
    async def run_model_index(agent_index):
        ...
        for _, row in sdf_subset.iterrows():
            for k in range(1):  # iterations per prompt

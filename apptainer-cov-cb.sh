...
#SBATCH --output=log-cb-train-%j.log

# ---------------------------------------------------------------------------
# Usage:
#   sbatch apptainer-train-cb.sh            # auto-resume from latest checkpoint
#   sbatch apptainer-train-cb.sh --force    # start from scratch
# ---------------------------------------------------------------------------
...
apptainer exec --nv env.sif \
  torchrun \
    ...
    --nproc_per_node=1 \
    train/train_cb.py --config train/config_cb.yaml \
    $FORCE_FLAG

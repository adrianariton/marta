...
safety_df = HarmBench().get_first(n=400)
...
BASE_GRAPHS_FOLDER = f"graphs/inpaphb/"
...
def plc(d):
    ddd = d.copy()
    ddd["category"] = (
        d["SemanticCategory"].map(lambda x: shorten(x))
        + ",\\\\"
        + d["FunctionalCategory"].map(lambda x: shorten(x))
    )
    return ddd

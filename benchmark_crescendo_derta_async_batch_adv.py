...
BASE_FOLDER = "datasets/crescendo_adv_gptoss120B_vs_derta__async_batch"
...
async def run_one(row):
    ...
    with MessageFileLogger(
        ...
        attacker = Yielder(logger, tag=Tag.ATTACK).of(
            StatelessSelfHostedOllamaAgent(host="dgxa100-ncit-wn01.grid.pub.ro", think="low")
        )
...
# BASE_FOLDER = "datasets/crescendo_adv_gemini_vs_derta__async_batch"
# with open(f"{BASE_FOLDER}/_choices.txt", "w+") as f:
#     _indices = safety_df[["id"]]
#     f.write(f"{_indices.to_dict()}")
...
#         agent = Yielder(logger, tag=Tag.AGENT).of(LlamaAgent(max_new_tokens=512).with_batch(batch))
#         attacker = Yielder(logger, tag=Tag.ATTACK).of(
#             StatelessRemoteGeminiAgent(model_name="gemini-2.5-pro")
#         )
#         evaluator = Yielder(logger, tag=Tag.EVAL).of(
#             AgentPEC7Evaluator(generator=StatelessRemoteClaudeAgent(max_tokens=1024))
#             # AgentPEC7Evaluator(generator=StatelessRemoteClaudeAgent(max_tokens=1024))

...
safety_df = LAODoNotAnswer().get_first(n=100)
batch = Batch(max_wait_seconds=3, max_batch_size=4)

BASE_FOLDER = "datasets/crescendo_lao_gemini_vs_circuitbreaker__async_batch"
...
async def run_one(row):
    ...
    with MessageFileLogger(
        output_folder_name=f"{BASE_FOLDER}/crescendo_lao_{row['id']}", buffer_max_size=20
    ) as logger:
        ...
        evaluator = Yielder(logger, tag=Tag.EVAL).of(
            AgentPEC7Evaluator(
                generator=StatelessRemoteClaudeAgent(
                    model_name="claude-haiku-4-5-20251001", max_tokens=1024
                )
            )
        ...
        with open(f"{BASE_FOLDER}/crescendo_lao_{row['id']}/info.txt", "w+") as f:

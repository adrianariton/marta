...
safety_df = XSTest().get_first(n=1000)[261:]
batch = Batch(max_wait_seconds=3, max_batch_size=4)

BASE_FOLDER = "datasets/crescendo_xstest_gptoss120B_vs_derta__async_batch"
...
async def run_one(row):
    ...
    with MessageFileLogger(
        output_folder_name=f"{BASE_FOLDER}/crescendo_xstest_{row['id']}", buffer_max_size=20
    ) as logger:
        agent = Yielder(logger, tag=Tag.AGENT).of(LlamaAgent(max_new_tokens=512).with_batch(batch))
        attacker = Yielder(logger, tag=Tag.ATTACK).of(
            StatelessSelfHostedOllamaAgent(host="dgxa100-ncit-wn02.grid.pub.ro", think="low")
        )
        ...
        with open(f"{BASE_FOLDER}/crescendo_xstest_{row['id']}/info.txt", "w+") as f:

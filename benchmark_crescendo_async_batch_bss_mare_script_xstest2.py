...
safety_df = XSTest().get_first(n=1000)
batch = Batch(max_wait_seconds=100, max_batch_size=30)
batch1 = Batch(max_wait_seconds=100, max_batch_size=30)
batch2 = Batch(max_wait_seconds=100, max_batch_size=30)
batch3 = Batch(max_wait_seconds=100, max_batch_size=30)
batch4 = Batch(max_wait_seconds=100, max_batch_size=30)
batch5 = Batch(max_wait_seconds=100, max_batch_size=30)


batch_vllm1 = Batch(max_wait_seconds=100, max_batch_size=30, name="VLLM1")
batch_vllm2 = Batch(max_wait_seconds=100, max_batch_size=90, name="VLLM2")
VLLM_HOST_BAGEL = "dgxh100-precis-wn03.grid.pub.ro"
...
def get_base_folder(index):
    if index == 0:
        return (
            "datasets/crescendo_xstest_qwen25_vs_circuitbreaker__async_batch_bagel_eval_3repl_3x8"
        )
    elif index == 1:
        return "datasets/crescendo_xstest_qwen25_vs_derta__async_batch_bagel_eval_3repl_3x8"
    elif index == 2:
        return "datasets/crescendo_xstest_qwen25_vs_door__async_batch_bagel_eval_3repl_3x8"
    elif index == 3:
        return "datasets/crescendo_xstest_qwen25_vs_guardllama__async_batch_bagel_eval_3repl_3x8"
    elif index == 4:
...
if __name__ == "__main__":
    results = asyncio.run(run_all(safety_df, max_concurrent=60))

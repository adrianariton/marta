...
Run:
    torchrun --nproc_per_node=3 eval_cb.py --config config_cb.yaml
    torchrun --nproc_per_node=3 eval_cb.py --config config_cb.yaml --force
"""
...
def main():
    ...
    # Tokenizer
    # ------------------------------------------------------------------
    print(f"[init] Tokenizer: {train_cfg['output_dir']}/best")
    tokenizer = AutoTokenizer.from_pretrained(
        f"{train_cfg['output_dir']}/best",
        padding_side="left",
        use_fast=False,
    )
    ...
    training_args = TrainingArguments(
        output_dir="/tmp/cb_eval_scratch",
        save_strategy="no",
        seed=seed,
        ...
        do_train=False,
        do_eval=True,
        eval_strategy="no",  # Tells HF not to look for intervals
        report_to="none",    # Disable tracking plugins
    ...
    class CBTrainer(Trainer):
        ...
        def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
            ...
            self.model_frozen.to(device)
            self.model_frozen_base.to(device)
        ...
        def evaluation_loop(
            ...
            if output.predictions is not None:
                # Based on your logs, output.predictions is already the dictionary of arrays
                metrics_dict = output.predictions
                print(f"[eval] Evaluation metrics: {metrics_dict}")
                aggregated = {}
                
                # Safely extract metrics_dict if it's wrapped in a tuple (sometimes happens in HF)
                if isinstance(metrics_dict, tuple):
                    metrics_dict = metrics_dict[0]

                # Iterate directly over the keys of the dictionary ('loss', 'cb_loss', etc.)
                for key, vals in metrics_dict.items():
                    # If it's a PyTorch Tensor, move to CPU and average it
                    if isinstance(vals, torch.Tensor):
                        mean_val = vals.float().mean().item()
                    # If it's a NumPy array, use numpy's mean
                    elif hasattr(vals, "mean"):
                        mean_val = vals.mean().item()
                    # Fallback for standard Python lists/numbers
                    else:
                        mean_val = sum(vals) / len(vals)

                    aggregated[f"{metric_key_prefix}_{key}"] = mean_val

                output.metrics.update(aggregated)
    ...
    print(f"[eval] Starting evaluation loop on dataset...")
    
    # --- CHANGED: Call evaluate instead of train ---
    metrics = trainer.evaluate()
    
    if trainer.args.local_rank in (-1, 0):
        print("\n" + "="*40)
        print("FINAL VERIFIED EVALUATION METRICS:")

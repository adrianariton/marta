...
safety_df = AdvBench().get_first(n=100)
batch = Batch(max_wait_seconds=3, max_batch_size=5)
other_batch = Batch(max_wait_seconds=3, max_batch_size=5)
eval_batch = Batch(max_wait_seconds=3, max_batch_size=5)

BASE_FOLDER = "datasets/goat_adv_mistral_vs_circuitbreaker__async_batch"
...
async def run_one(row):
    ...
    with MessageFileLogger(
        output_folder_name=f"{BASE_FOLDER}/goat_adv_{row['id']}", buffer_max_size=20
    ) as logger:
        ...
        attacker = Yielder(logger, tag=Tag.ATTACK).of(
            MistralAgent(
                model="mistralai/Mistral-Small-24B-Instruct-2501", max_new_tokens=2048
            ).with_batch(other_batch)
            # StatelessRemoteGeminiAgent()
        )
        evaluator = Yielder(logger, tag=Tag.EVAL).of(
            ...
            AgentPEC7Evaluator(
                generator=MistralAgent(
                    model="mistralai/Mistral-Small-24B-Instruct-2501", max_new_tokens=1024
                ).with_batch(eval_batch)
            )
        )
        ...
        with open(f"{BASE_FOLDER}/goat_adv_{row['id']}/info.txt", "w+") as f:
...
async def run_all(safety_df, max_concurrent: int = 5):
    ...
    await batch.end()
    await other_batch.end()
    await eval_batch.end()
    return all_tasks


if __name__ == "__main__":
    results = asyncio.run(run_all(safety_df, max_concurrent=10))

...
def main():
    ...
    retain_texts = build_ultrachat_texts(
        tokenizer,
        n_samples=data_cfg.get("ultrachat_retain_n", 1000),
        seed=data_cfg.get("ultrachat_seed", 42),
    )

    # Oversample harmful texts to balance classes
    oversample_factor = data_cfg.get("harmful_oversample_factor", 3)
    harmful_col = data_cfg["harmful_col"]
    harmful_mask = train_df[harmful_col].astype(bool)
    harmful_texts = build_safety_texts(train_df[harmful_mask].copy(), tokenizer, data_cfg, seed + 2)
    oversampled = harmful_texts * (oversample_factor - 1)  # already in train_texts once

    all_train = train_texts + oversampled + retain_texts
    random.Random(seed).shuffle(all_train)

    # print("Sample train_texts:")
    # for i, text in enumerate(all_train[:5]):
    #     print(f"  {i}: {text}")

    # print(f"Sample retain_texts:")
    # for i, text in enumerate(retain_texts[:5]):
    #     print(f"  {i}: {text}")
    ...
    # ------------------------------------------------------------------
    # Weighted loss — penalize unsafe errors more
    # ------------------------------------------------------------------
    unsafe_weight = train_cfg.get("class_weight_unsafe", 4.0)

    # Token ids for "safe" and "unsafe" in model vocab
    ...
    class WeightedSFTTrainer(SFTTrainer):
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            outputs = model(
                **{k: v for k, v in inputs.items() if k != "labels"}, labels=inputs["labels"]
            )
            logits = outputs.logits  # (B, L, V)
            labels = inputs["labels"]  # (B, L)

            # Build per-token weight: unsafe_weight where label==unsafe_id, else 1.0
            weights = torch.ones_like(labels, dtype=logits.dtype)
            weights[labels == unsafe_id] = unsafe_weight
            ...
            loss = (loss * shift_weights.view(-1)).sum() / (
                shift_weights.view(-1)[shift_labels.view(-1) != -100]
            ).sum().clamp(min=1)
